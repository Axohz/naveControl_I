...
ship_pos = None
ship_fuel = 0
visited_edges = defaultdict(int)  # Diccionario para contar las visitas a cada arista
...
def move_ship_along_path(path, steps):
    global ship_pos, ship_fuel, visited_edges
    ...
    while to_move > 0 and idx < len(path) - 1:
        ...
        ship_fuel -= cost
        visited_edges[(u, v)] += 1
        visited_edges[(v, u)] += 1  # Contar ambas direcciones
        ship_pos = v
        idx += 1
        to_move -= 1
...
def viaje_manual_hacia(dest):
    ...
    if ship_fuel < cost:
        ...
        sys.exit(1)
    ship_fuel -= cost
    visited_edges[(ship_pos, dest)] += 1
    visited_edges[(dest, ship_pos)] += 1  # Contar ambas direcciones
    ship_pos = dest
...
def obtener_color_arista(visit_count):
    if visit_count == 1:
        return "red"
    elif visit_count == 2:
        return "darkred"
...
# Dibujar las aristas visitadas con colores según visitas
for edge, count in visited_edges.items():
    nx.draw_networkx_edges(G, pos, edgelist=[edge], edge_color=obtener_color_arista(count), width=2.5)
